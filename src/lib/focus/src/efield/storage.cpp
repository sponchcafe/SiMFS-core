...
namespace sim{
    namespace field{
        ...
        bool load_field_file(std::string fname, EFieldSpec &header, std::vector<EFieldComponents> &t){
            ...
            fs.close();
            std::cerr << "Successfully loaded field file " << fname << " - ";
            std::cerr << "(" << header.x.n << " x " << header.y.n << " x " << header.z.n << ")\n";
            return true;
        ...
        double get_grid_step(EFieldDimension d){
            if (d.n > 1) return (d.max-d.min) / (d.n-1);
            return 0;
        }
        ...
        void make_grid_coordinates(EFieldSpec spec, std::vector<EFieldCoordinate> &t){
            ...
            double x_step = get_grid_step(spec.x);
            double y_step = get_grid_step(spec.y);
            double z_step = get_grid_step(spec.z);
