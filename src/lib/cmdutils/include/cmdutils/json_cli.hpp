        ...
        class JsonCLI{
            ...
            public:
                ...
                JsonCLI(int argc, char* argv[], char** envp){
                    args[META_KEY]["execname"] = argv[0];
                    args[SHELL_KEY] = parse_argv_vector(argc, argv);
                    args[ENV_KEY] = parse_envp_vector(envp);
                }
                ...
                json expand_env_var(std::string key){
                    std::string value= ""; 
                    try{
                        value = args[ENV_KEY][key];
                    } catch (std::exception e){
                        std::cerr << "Environment variable "; 
                        std::cerr << key << " not found" << std::endl;
                        return json{};
                    }
                    json jvalue = string_to_json(value);
                    args[ENV_KEY][key] = jvalue;
                    return jvalue;
                }
            ...
            private:
                ...
                bool is_file_specifier(std::string s){
                    return (s.length() > 1) && 
                        (s.substr(0, FILE_TOKEN.length()) == FILE_TOKEN);
                }
                ...
                json string_to_json(std::string s){
                    ...
                    if (is_file_specifier(s)){
                        entry = file_to_json(strip_file_specifier(s));
